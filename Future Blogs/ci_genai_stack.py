...
class CdkappStack(Stack):
    ...
    # create a vpc with IpAddresses 10.10.0.0/16, a NAT gateway, a public subnet, PRIVATE_WITH_EGRESS subnet and a RDS subnet
        ...
        # create an Amazon Linux 2023 image
        amzn_linux = ec2.MachineImage.latest_amazon_linux(generation = ec2.AmazonLinuxGeneration.AMAZON_LINUX_2023
                        )

        # read userdata file from cdkapp directory
        with open("./cdkapp/userdata.sh","r") as script_file:
            script_lines = script_file.readlines()

        # Add each line from the script to UserData
        user_data = ec2.UserData.for_linux()
        user_data.add_commands(*[line.strip() for line in script_lines])
